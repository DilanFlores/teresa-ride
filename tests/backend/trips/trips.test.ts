...
import * as tripsService from '../../../backend/src/services/trips.service';
import { Trip } from '../../../backend/src/models/trip.model';
...
describe('trips.service', () => {
    ...
    test('getAllTrips retorna lista', async () => {
        ...
        queryMock.mockImplementation((sql: string, cb: Function) =>
            cb(null, rows)
        );
    ...
    test('getTripById retorna 1 registro', async () => {
        ...
        queryMock.mockImplementation(
            (sql: string, params: any[], cb: Function) => cb(null, [row])
        );
    ...
    test('getTripById rechaza si no existe', async () => {
        queryMock.mockImplementation(
            (sql: string, params: any[], cb: Function) => cb(null, [])
        );

        await expect(tripsService.getTripById('999'))
            .rejects
    ...
    test('createTrip inserta y retorna el trip', async () => {
        const newTrip: Trip = {
            ...
            people_count: 3,
            description: 'Viaje nuevo',
            image: ''
        };

        queryMock.mockImplementationOnce(
            (sql: string, params: any, cb: Function) =>
                cb(null, { insertId: 3 })
        );
    ...
    test('updateTrip rechaza si no hay registros afectados', async () => {
        ...
        await expect(tripsService.updateTrip('1', { price: 250 }))
            .rejects
            .toThrow('Trip not found');
    });
    ...
    test('deleteTrip rechaza cuando affectedRows = 0', async () => {
        ...
        await expect(tripsService.deleteTrip('999'))
            .rejects
            .toThrow('Trip not found');
    });
