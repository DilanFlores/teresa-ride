...
import * as reviewService from '../../../backend/src/services/review.service';
...
describe('review.service', () => {
    ...
    test('updateReview rechaza si no existe', async () => {
        ...
        await expect(reviewService.updateReview('2', { comment: 'new' }))
    ...
    test('deleteReview rechaza cuando affectedRows = 0', async () => {
        ...
        await expect(reviewService.deleteReview('2'))
            .rejects.toThrow('Review not found');
    });

    test('createReview rechaza rating null', async () => {
        const payload = { id_reservation: 'R1', comment: 'x', rating: null, date_review: new Date(), type: 'Vehicle' } as any;

        await expect(reviewService.createReview(payload))
            .rejects.toThrow('rating must be between 1 and 5');
    });

    test('createReview rechaza rating menor que 1', async () => {
        const payload = { id_reservation: 'R1', comment: 'x', rating: 0, date_review: new Date(), type: 'Vehicle' } as any;

        await expect(reviewService.createReview(payload))
            .rejects.toThrow('rating must be between 1 and 5');
    });

    test('createReview rechaza rating mayor que 5', async () => {
        const payload = { id_reservation: 'R1', comment: 'x', rating: 6, date_review: new Date(), type: 'Vehicle' } as any;
